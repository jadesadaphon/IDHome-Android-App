...
class LoginApi {
    suspend fun loginApi(context: Context, code: String, password: String): JSONObject? {
            ...
            try {
                ...
                client.newCall(request).execute().use { response ->
                    if (response.isSuccessful) {
                        ...
                        Log.e("LoginApi", "HTTP response status codes : ${response.code}")
...
//                        }
                ...
                Log.e("LoginApi", "Error: ${e.message}")
                withContext(Dispatchers.Main) {
                    val sound = MediaPlayer.create(context, R.raw.error_when_entering_the_game_menu)
                    val strMessage = HtmlCompat.fromHtml(String.format(context.getString(
                        R.string.failed_to_connect_message),"ไอดีโฮม2015"), HtmlCompat.FROM_HTML_MODE_COMPACT)
                    val strTitle = context.getString(R.string.failed_to_connect)
                    val builder: AlertDialog.Builder = AlertDialog.Builder(context)
                    builder
                        .setMessage(strMessage)
                        .setTitle(strTitle)
